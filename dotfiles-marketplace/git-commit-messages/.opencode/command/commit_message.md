...
This command analyses the specified commit, creates an improved commit message following Conventional Commits specification.

## Overview

This command analyses git commits and generates improved commit messages following the Conventional Commits specification. The message should be concise and focus on business value, not implementation details.

Load the `conventional-commits` skill for the complete specification.
...
1. **Analyse the Commit**: Use `git show` with the commit reference:
   - Examine commit metadata (author, date, hash)
   - View the full diff of changes
   - Display the current commit message

2. **Understand Context**: Use `git log`:
...
4. **Write to File**: Save the commit message to `.commit-message.txt`:
...
**Example 1: Analyse most recent commit**
```
/commit_message HEAD
```

**Example 2: Analyse specific commit**
```
/commit_message abc1234
```

**Example 3: Analyse commit from two commits back**
```
