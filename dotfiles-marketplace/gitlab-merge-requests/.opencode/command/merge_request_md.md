...
1. Analysing changes using git tools
...
- **If empty or "HEAD"**: Use `git show` to analyse the latest commit
- **If commit range**: Use `git diff` to see full changes
- **If branch name**: Use `git diff` comparing against target branch

Additionally:
- **`git log`**: Review recent commits for context
...
Write the complete content to `MERGE_REQUEST.md` with proper markdown formatting.
