...
must_haves:
  truths:
    ...
    - "Age private key is NOT stored in the chezmoi source directory (bootstrap key lives only in Bitwarden + local filesystem)"
  artifacts:
...
**IMPORTANT DESIGN DECISION — Age keys are NOT stored in chezmoi source:**

The age private key file (`~/.config/age/key-<machine_type>.txt`) must NEVER be added to the chezmoi source directory. This is by design:

- The age key is the bootstrap key that decrypts everything else in the chezmoi repo
- Storing the decryption key alongside the encrypted files would defeat the purpose of encryption
- The age key lives in exactly two places: (1) local filesystem at `~/.config/age/` and (2) Bitwarden vault as a Secure Note
- On a new machine, the bootstrap flow is: install chezmoi -> unlock Bitwarden -> retrieve age key -> save to `~/.config/age/` -> `chezmoi apply` decrypts SSH keys
- Do NOT run `chezmoi add ~/.config/age/` — if chezmoi tries to manage this path, explicitly add it to `.chezmoiignore`

Verify age key is excluded:
```bash
grep -q "age" ~/.local/share/chezmoi/.chezmoiignore 2>/dev/null || echo ".config/age" >> ~/.local/share/chezmoi/.chezmoiignore
```

This ensures `chezmoi add` or `chezmoi re-add` never accidentally picks up the age key.
  </action>
  <verify>
...
6. `grep "age" ~/.local/share/chezmoi/.chezmoiignore` — age directory is excluded from chezmoi management
7. `ls ~/.local/share/chezmoi/private_dot_config/age/ 2>/dev/null` — should NOT exist (age key not in source)
  </verify>
  <done>Age encryption configured in chezmoi. SSH private keys encrypted in source. Age key pair generated for current machine type. Age private key explicitly excluded from chezmoi source (lives only in local filesystem + Bitwarden).</done>
</task>
...
5. Age private key NOT present in chezmoi source directory (excluded via .chezmoiignore)
6. `chezmoi apply` successfully decrypts and applies SSH keys
</verification>
...
- Age private key excluded from chezmoi source (bootstrap key design)
